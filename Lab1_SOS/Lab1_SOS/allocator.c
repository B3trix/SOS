...
UINT   allocType = MEM_COMMIT; //allocate memory charges for the specified reserved memory pages
UINT   allocProtect = PAGE_READWRITE; //read-only access to the committed region of pages
...
// Unlock and free the specified region of the process's virtual address space into physical memory
...
// Diagnostic and debugging output (invalid address)
bool checkAddressRange(void* _object)
{
	...
	if (object >= (uintptr_t)memory && object < ((uintptr_t)memory + (uintptr_t)memory_size)) {
		...
		fprintf(stderr, "checkAddressRange ERROR: Invalid Address\n");
		return false;
	}
}
...
int mem_init(size_t size) {
	...
	mem_heap.size = size;
	mem_block_header_t* block = (mem_block_header_t*)malloc(sizeof(mem_block_header_t));
